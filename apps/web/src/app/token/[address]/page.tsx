...
export default function TokenPage() {
  ...
  return <section className="page shell">
    ...
    <div className="detail-grid"><div className="panel chart-panel"><PriceChart tokenAddress={item.address} /></div><TradePanel tokenAddress={item.address} pairAddress={item.liquidityPool?.address ?? item.liquidityPool?.pairAddress} decimals={item.decimals} disabled={item.status === "GRADUATED"} /></div>
    ...
    <div className="tables-grid"><section className="panel"><div className="panel-heading"><h2>Recent trades</h2><span>{trades.data?.meta.total ?? 0} total</span></div>{trades.isPending ? <div className="table-loading skeleton" /> : trades.data?.data.length ? <div className="data-table">{trades.data.data.map((trade) => <div className="table-row" key={trade.id}><span className={trade.side === "BUY" ? "buy" : "sell"}>{trade.side}</span><span>{formatAddress(trade.walletAddress)}</span><b>{formatTokenAmount(trade.tokenAmount, item.decimals)} {item.symbol}</b><time>{new Date(trade.blockTimestamp).toLocaleDateString()}</time></div>)}</div> : <p className="table-empty">No trades indexed yet.</p>}</section><section className="panel"><div className="panel-heading"><h2>Top holders</h2><span>{holders.data?.meta.total ?? 0} total</span></div>{holders.isPending ? <div className="table-loading skeleton" /> : holders.data?.data.length ? <div className="data-table">{holders.data.data.map((holder, index) => <div className="table-row holder-row" key={holder.walletAddress}><span>#{index + 1}</span><span>{formatAddress(holder.walletAddress)}</span><b>{formatTokenAmount(holder.balance, item.decimals)}</b><time>{(holder.ownershipBps / 100).toFixed(2)}%</time></div>)}</div> : <p className="table-empty">No holders indexed yet.</p>}</section></div>
  </section>;
}
