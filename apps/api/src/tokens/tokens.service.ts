...
export class TokensService {
  ...
  async list(
    ...
    const chainKey = query.chainId ?? 'all';

    const key = `api:tokens:v${version}:${chainKey}:${query.status ?? 'all'}:${query.sort}:${query.page}:${query.limit}`;
  ...
  async findOne(
    ...
    const key = `api:token:v${version}:${chainId}:${address}`;

    const cached =
      await this.redis.getJson<Record<string, unknown>>(key);
  ...
  async trades(
    ...
    await this.ensureToken(address, query.chainId);
  ...
  async candles(
    ...
    const key =
      `api:candles:v${version}:${query.chainId}:${address}:${query.interval}:` +
    ...
    const result =
      serializeValue(rows.reverse()) as Record<string, unknown>[];
