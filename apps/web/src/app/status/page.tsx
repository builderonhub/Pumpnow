...
async function health(url: string): Promise<Health> {
  ...
  if (!response.ok) {
    throw new Error(`${body.service ?? "Service"} is unavailable`);
  }
  return body;
}
...
export default function StatusPage() {
  const api = useQuery({
    queryKey: ["health", "api"],
    queryFn: () => health(`${API_URL}/api/health`),
    refetchInterval: 30_000,
    retry: 2,
  });
  const indexer = useQuery({
    queryKey: ["health", "indexer"],
    queryFn: () => health(`${indexerUrl}/health`),
    refetchInterval: 30_000,
    retry: 2,
  });
  ...
  const services = [
    { name: "API", query: api },
    { name: "Indexer", query: indexer },
  ];

  return (
    <section className="page shell">
      <div className="page-intro compact">
        <span className="kicker">TESTNET BETA</span>
        <h1>System status</h1>
        <p>Live readiness checks refresh every 30 seconds.</p>
      </div>
      <div className="token-grid">
        {services.map(({ name, query }) => {
          const tone = query.isPending
            ? "checking"
            ...
            <article className="panel" key={name}>
              ...
              {query.data?.checks
                ? Object.entries(query.data.checks).map(([key, value]) => (
                    <div className="progress-label" key={key}>
                      <span>{key}</span>
                      <b>{formatCheck(value)}</b>
                    </div>
                  ))
                : null}
              {query.error ? (
